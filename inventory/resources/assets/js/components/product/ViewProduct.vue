<template>
  <div class="wrap">
    <div class="body">
      <div class="row">

        <div class="col-md-4">
          <input
            type="text"
            class="form-control"
            v-on:keyup="getData"
            placeholder="Buscar por nombre"
            name
            v-model="name"
          />
        </div>

        <div class="col-md-4">
          <select class="form-control select2" @change="getData" v-model="cat" v-select="cat">
            <option value>Filtrar por categoría</option>
            <option v-for="(value,index) in categorys" :value="value.id">{{ value.name }}</option>
          </select>
        </div>

        <div class="col-md-4">
          <select class="form-control select2" @change="getData" v-model="uni" v-select="uni">
            <option value>Filtrar por unidad</option>
            <option v-for="(value,index) in units" :value="value.id">{{ value.customer_name }}</option>
          </select>
        </div>

        <div class="col-md-4">
          <input
            type="text"
            class="form-control"
            v-on:keyup="getData"
            placeholder="Buscar por marca"
            name
            v-model="marca"
          />
        </div>

        
        
      </div>

      <div class="loading" v-if="isLoading">
        <h2 style="text-align:center">Cargando.....</h2>
      </div>

      <div class="table-responsive" v-else>
        <table class="table table-condensed table-hover">
          <thead>
            <tr>
              <th>Nombre</th>
              <th>Categoría</th>
              <th>Marca</th>
              <th>Detalles</th>
              <th>Stock</th>
              <th>Stock crítico</th>
              
              <th v-if="ocultar_columna" style="text-align: center">Editar</th>
              <th v-if="ocultar_columna" style="text-align: center">Eliminar</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(value,index) in products.data">
              <td>{{ value.product_name }}</td>
              <td>{{ value.category.name }}</td>
              <td>{{ value.marca }}</td>
              <td>{{ value.details }}</td>
              <td :class="{ 'text-danger': value.stock < value.critico }">{{ value.stock }}</td>
              <td>{{ value.critico }}</td>

              <td style="text-align: center">
                <button
                  v-if="ocultar_columna"
                  @click="editProduct(value.id)"
                  type="button"
                  class="btn btn-warning btn-circle waves-effect waves-circle waves-float"
                >
                  <i class="material-icons">edit</i>
                </button>
              </td>
              <td style="text-align: center">
                <button
                  v-if="ocultar_columna"
                  @click="deleteProduct(value.id)"
                  type="button"
                  class="btn bg-pink btn-circle waves-effect waves-circle waves-float"
                >
                  <i class="material-icons">delete</i>
                </button>
              </td>
            </tr>
          </tbody>
        </table>
      </div>

      <pagination :pageData="products"></pagination>

      <div class="row">
        <update-product :cat="categorys"></update-product>
        <update-product :uni="units"></update-product>
      </div>
    </div>
  </div>
</template>

<script>
import { EventBus } from "../../vue-asset";

import mixin from "../../mixin";

import UpdateProduct from "./UpdateProduct.vue";

import Pagination from "../pagination/pagination.vue";

import Swal from 'sweetalert2';

export default {
  props: ["categorys","units"],

  mixins: [mixin],

  components: {
    "update-product": UpdateProduct,
    pagination: Pagination,
  },

  data() {
    return {
      products: [],
      name: "",
      marca: "",
      cat: "",
      uni: "",
      isLoading: true,
      sedeLS: '',
      ocultar_columna: true,
      imageCache: {}
    };
  },
  created() {

    this.sedeLS = localStorage.getItem('rol');

    if(this.sedeLS === "maestro"){
      this.ocultar_columna = false;
    }

    var _this = this;
    this.getData();

    EventBus.$on("product-created", function () {
      window.history.pushState({}, null, location.pathname);
      _this.getData();
    });
  },

  methods: {

    showImageInSweetAlert(image) {
      Swal.fire({
        imageUrl: image,
        imageWidth: 400,
        imageHeight: 300,
        confirmButtonText: 'Cerrar',
        confirmButtonClass: 'bg-teal'
      });
    },

    getData(page = 1) {
      this.isLoading = true;
      axios
        .get(
          base_url +
            "product-list?page=" +
            page +
            "&name=" +
            this.name +
            "&marca=" +
            this.marca +
            "&cat=" +
            this.cat +
            "&uni=" +
            this.uni
        )
        .then((response) => {
          this.products = response.data;
          this.isLoading = false;
        })
        .catch((error) => {
          console.log(error);
        });
    },

    editProduct(id) {
      EventBus.$emit("product-edit", id);
    },

    showMessage(data) {
      if (data.status == "success") {
        toastr.success(data.message, "Success Alert", { timeOut: 500 });
      } else {
        toastr.error(data.message, "Error Alert", { timeOut: 500 });
      }
    },

    deleteProduct(id) {
      Swal(
        {
          title: "¿Estás seguro?",
          text: "¡No podrás revertir esto!",
          type: "warning",
          showCancelButton: true,
          confirmButtonColor: "#3085d6",
          cancelButtonColor: "#d33",
          cancelButtonText: "Cancelar",
          confirmButtonText: "¡Sí, eliminar!",
        },
        () => {}
      ).then((result) => {
        if (result.value) {
          axios.get(base_url + "product/delete/" + id).then((res) => {
            EventBus.$emit("product-created", 1);

            this.successALert(res.data);
          });
        }
      });
    },
    pageClicked(pageNo) {
      var vm = this;
      vm.getData(pageNo);
    },
  },

  computed: {},
};
</script>

<style>
  .text-danger {
    color: red;
    font-weight: bold;
  }
</style>